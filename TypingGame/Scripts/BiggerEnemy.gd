extends Sprite # Bigger Enemy script

export (Color) var blue = Color("#0000ff")
export (Color) var green = Color("#00ff00")
export (Color) var red = Color("#ff0000")
export (float) var speed = 0.9
export (float) var scale_factor = 1.3
...
func _ready() -> void:
	prompt_text = PromptList.get_phrase_prompt()
...
func _physics_process(_delta: float) -> void:
	...
	if global_position.x <= game_wall.rect_global_position.x + game_wall.rect_size.x/2:
		...
		player_health.take_damage() # Lose 2 hearts for bigger enemy
		queue_free()
